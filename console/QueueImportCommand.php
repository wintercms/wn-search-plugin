...
#[AsCommand(name: 'search:queue-import')]
class QueueImportCommand extends BaseQueueImportCommand
{
    /**
     ...
     * @var string
     */
    protected $signature = 'search:queue-import
            {model : Class name of model to bulk queue}
            {--min= : The minimum ID to start queuing from}
            {--max= : The maximum ID to queue up to}
            {--c|chunk= : The number of records to queue in a single job (Defaults to configuration value: `search.chunk.searchable`)}
            {--queue= : The queue that should be used (Defaults to configuration value: `search.queue.queue`)}';
    ...
    public function handle()
    {
        ...
        $chunk = max(1, (int) ($this->option('chunk') ?? config('search.chunk.searchable', 500)));
